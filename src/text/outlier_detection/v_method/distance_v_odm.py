...
class DistanceV_ODM(V_ODM):
    def __init__(self, dataset, space: Space, use_cached=False,
                 output_path=None, odm_model: NumericalVOdmAdapter = None, **params):
        if odm_model is None:
            odm_model = VMMDAdapter()
        super().__init__(dataset,
                         use_cached=use_cached, classifier_delta = 0.0, output_path=output_path,odm_model= odm_model, space=space, **params)
